...
function onTick()
	...
	if timer >= lightning.timer then 
		...
		if timer == lightning.timer + 2 then
			if (player.sectionObj.backgroundID == 31 or player.sectionObj.backgroundID == 32) then
...
function onDraw() --onDraw runs even when the game is paused, onTick doesn't
	if (player.sectionObj.backgroundID == 31 or player.sectionObj.backgroundID == 32) and timer >= lightning.timer then
