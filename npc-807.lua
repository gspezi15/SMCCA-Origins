...
function sampleNPC.onTickNPC(v)
	...
	for _,p in NPC.iterateIntersecting(v.x - 4, v.y - 4, v.x + v.width + 4, v.y + v.height + 4) do
		...
		if p:mem(0x12A, FIELD_WORD) > 0 and p:mem(0x138, FIELD_WORD) == 0 and (not p.isHidden) and (not p.friendly) and p:mem(0x12C, FIELD_WORD) == 0 and p.idx ~= v.idx then
			v:kill(HARM_TYPE_OFFSCREEN)
			Explosion.spawn(v.x,v.y,2)
		end
	end
	
	for _,p in ipairs(Player.getIntersecting(v.x - 4, v.y - 4, v.x + v.width + 4, v.y + v.height + 4)) do
