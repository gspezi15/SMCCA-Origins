...
function leaper.register(id)
    npcManager.registerEvent(id, leaper, "onTickEndNPC")
...
function leaper.onTickEndNPC(v)
	...
	if data.animTimer <= 0 or data.animTimer >= 17 then
		v.animationFrame = 0
	else
		v.animationFrame = 1
	end
	...
	if v.ai1 > 0 then
		...
		if data.animTimer > 17 then
			...
			local projectile = NPC.spawn(NPC.config[v.id].projectile, originX, originY, targetedplayer.section, false, true)
			SFX.play(42)
