...
local sampleNPCSettings = {
	...
	grabside=false,
	grabtop=false,
	--This here contains a list NPC ids that CAN defeat Krusha, feel free to alter to your liking.
	NPCList = {45, 154, 155, 156, 157, 241, 50},
}
...
function sampleNPC.onNPCHarm(eventObj,v,reason,culprit)
	...
	if culprit then
		if culprit.__type == "Player" then
			...
			if reason == HARM_TYPE_TAIL or reason == HARM_TYPE_JUMP or reason == HARM_TYPE_SPINJUMP then
		...
		if culprit.__type == "NPC" then
			for k,t in ipairs(sampleNPCSettings.NPCList) do
				if culprit.id == t or NPC.config[culprit.id].isshell or NPC.config[culprit.id].isvegetable then
